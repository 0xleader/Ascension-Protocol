...
contract DistributeAscendStrategy is IAscensionStrategy {
  //VERSION 1.0.0
  string public name = "DistributeAscendStrategy";
  //STATE
    ...
    returns (bool)
  {
    ...
    //how much ASCEND did we receive?
    uint256 tokens = IERC20(ascend).balanceOf(address(this));

    //Distribute tokens to all holders
    IAscension(ascend).distributeShares(tokens);

    emit ProposalExecuted(name, ethAmount, tokens);
    return true;
  }
}
