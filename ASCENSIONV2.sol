...
contract ASCENSIONV2 is Context, IERC20, IAscension {
  ...
  function lockLiquidity() external override {
    ...
    //subtract caller reward from contract shares
    _shares[address(this)] = _shares[address(this)].sub(callerReward);
    //add reward shares to caller shares
    _shares[caller] = _shares[caller].add(callerReward);

    uint256 half = _shares[address(this)].div(2);
    uint256 otherHalf = _shares[address(this)].sub(half);
